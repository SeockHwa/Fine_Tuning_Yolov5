...
###############################################################################################################################################
    ...
    #                 iou_li.append(iou)
###############################################################################################################################################
    for j in data_list[1]:  # 현재 사진의 차량 박스와 비교
        for v in absdiff_list:  # iou 비교
            iou = get_iou(j, v)
            if iou > opt.abs_iou: #abs_iou 사전에 정의함
                iou_list.append(j)
                # print(j[3])  # iou 리스트의 y축 수치 출력
                iou_li.append(iou)
                print(iou_li)
    ...
    parking_place = opt.parkingplace
    frame__ = cv2.imread(parking_place)

    pts_src = np.array([[7443,1699],[7163,1427],[7945,1425],[7943,1699]])
    pts_dst = np.array([[263,122],[424,91],[650,405],[54,406]])

    check_parking(frame_, frame__, iou_list, pts_src, pts_dst)
...
# def check_parking(img, xyxy,src,dst):
#     imgsz = img.size
#     center_linex = 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--before', type=str, default='data/images/def/12_2.png', help='before images')
    parser.add_argument('--after', type=str, default='data/images/def/80_line.png', help='after images')
    parser.add_argument('--parkingplace', type=str, default='data/images/def/ch_b1_point.png', help='parking place images')
