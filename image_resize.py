...
#######################################################################################################################################
    for path, img, im0s, vid_cap in dataset:
        im0s = cv2.resize(im0s, dsize=(720,480))
...
#####################################################################################################################################################################
        ...
        for i, det in enumerate(pred):  # detections per image 
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image #이미지에 바운딩 박스 그리기
                        ...
                        if names[int(cls)] == 'car': #클래스가 CAR일 경우
                            ...
                            plot_one_box(xyxy_, im0, label=label, color=colors[int(cls)], line_thickness=2)
            ...
            new_path = '/Users/shjeong/Desktop/mtov/yolov5/data/images/input_image/4' + '/' + str(tmp) + '.png'
            frame_tmp = cv2.imread(new_path)  # 현재 사진 로딩
            frame_tmp = cv2.resize(frame_tmp, dsize=(720,480))
            
            #point_list = ROI.Roi.point_list_image
            #check_inout(frame_,data_list[0],point_list)
            #check_inout1(frame_,data_list[0],point_list,ROI.RoI_region.ROI_list_image)
            ...
            for x1,y1,x2,y2 in data_list_tmp:
                ...
                if k == 27:
                    ...
                    cv2.imwrite('data/images/save_image/saveimg{}.png'.format(count_q), resize_img)
                    count_q += 1
                    cv2.destroyAllWindows()
...
###############################################################################################################################################
    ...
    # 사전에 설정해둔 주차면 데이터 범위 값 배열

    # for a in iou_list:  # a = iou 순서
    #     print(a)
    ...
    # print('in count ' + str(len(iou_list)))
    # print('out count ' + str(len(iou_list) - after_count))
...
def absdiff():
    ...
    ground = cv2.resize(ground, dsize = (720, 480))
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--source', type=str, default='data/images/input_image/1.png', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--source', type=str, default='data/images/input_image/4', help='source')  # file/folder, 0 for webcam
    ...
    # parser.add_argument('--before', type=str, default='data/images/def/images4_3.png', help='before images')
    # parser.add_argument('--after', type=str, default='data/images/def/images4_3.png', help='after images')
